...
void runExperiment() {
    std::vector<int> sizes = {3, 4, 5, 6, 7, 8, 9, 10};  // Размеры алфавита
    std::vector<double> timesGetAllPerms;
    std::vector<double> timesGetPerm1;
    std::vector<double> timesGetPerm2;
    
    std::cout << "Запуск эксперимента по измерению времени..." << std::endl;
    std::cout << "----------------------------------------" << std::endl;
    ...
    for (int n : sizes) {
        ...
        std::cout << "Размер n = " << n << ", количество перестановок: " << factorial(n) << std::endl;
        
        auto treeCreateStart = std::chrono::high_resolution_clock::now();
        PMTree tree(symbols);
        auto treeCreateEnd = std::chrono::high_resolution_clock::now();
        double treeCreateTime = std::chrono::duration<double, std::milli>(treeCreateEnd - treeCreateStart).count();
        std::cout << "  Время построения дерева: " << treeCreateTime << " мс" << std::endl;
        
        double getAllPermsTime = measureTime([&]() {
            getAllPerms(tree);
        });
        timesGetAllPerms.push_back(getAllPermsTime);
        std::cout << "  getAllPerms: " << getAllPermsTime << " мс" << std::endl;
        
        int permNum = getRandomPermNumber(factorial(n));
        std::cout << "  Случайный номер перестановки: " << permNum << std::endl;
        
        double getPerm1Time = measureTime([&]() {
            getPerm1(tree, permNum);
        });
        timesGetPerm1.push_back(getPerm1Time);
        std::cout << "  getPerm1 (обход): " << getPerm1Time << " мс" << std::endl;
        
        // Измеряем getPerm2 (навигация)
        double getPerm2Time = measureTime([&]() {
            getPerm2(tree, permNum);
        });
        timesGetPerm2.push_back(getPerm2Time);
        std::cout << "  getPerm2 (навигация): " << getPerm2Time << " мс" << std::endl;
        std::cout << "----------------------------------------" << std::endl;
    }
    ...
    if (outfile.is_open()) {
        ...
        outfile.close();
        std::cout << "\nРезультаты сохранены в result/times.csv" << std::endl;
    }
    ...
    if (plotScript.is_open()) {
        plotScript << "import matplotlib.pyplot as plt\n"
                   ...
                   << "plt.semilogy(data['n'], data['getAllPerms'], 'o-', linewidth=2, markersize=8, label='getAllPerms')\n"
                   << "plt.semilogy(data['n'], data['getPerm1'], 's-', linewidth=2, markersize=8, label='getPerm1 (обход)')\n"
                   << "plt.semilogy(data['n'], data['getPerm2'], '^-', linewidth=2, markersize=8, label='getPerm2 (навигация)')\n\n"
                   << "plt.xlabel('Размер алфавита (n)', fontsize=12)\n"
                   << "plt.ylabel('Время выполнения (мс)', fontsize=12)\n"
                   << "plt.title('Зависимость времени выполнения от размера алфавита', fontsize=14)\n"
                   << "plt.legend(fontsize=10)\n"
                   << "plt.grid(True, alpha=0.3)\n"
                   << "plt.savefig('plot.png', dpi=300, bbox_inches='tight')\n"
                   << "plt.show()\n";
        plotScript.close();
        std::cout << "Скрипт для построения графика создан: result/plot.py" << std::endl;
    }
}
...
void demonstrate() {
    ...
    std::cout << "\nПервые 5 перестановок для {1,3,5,7}:" << std::endl;
...
int main() {
    std::cout << "=== Генерация перестановок с помощью дерева вариантов ===" << std::endl;
